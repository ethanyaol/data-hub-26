...
import type { SubtaskRecord } from "./types";
...
const ViewSubtasksAgent = () => {
  ...
  // Filtering logic
  const filteredRecords = mockSubtasks.filter((record) => {
  ...
  const renderClaimStatus = (status: SubtaskRecord["claimStatus"]) => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
                    ...
                    <td>
                      {record.assignedPerson ? (
                        <button
                          ...
                          onClick={() =>
                            navigate(
                              `/dashboard/tasks/${taskId}/recovery/${agentId}/personnel`
                            )
                          }
                        >
                          {record.assignedPerson}
      ...
      <AlertDialog open={reassignDialogOpen} onOpenChange={setReassignDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>子任务删除分配人员</AlertDialogTitle>
            <AlertDialogDescription>
...
export default ViewSubtasksAgent;
