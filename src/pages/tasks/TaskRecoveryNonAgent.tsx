...
const TaskRecoveryNonAgent = () => {
  ...
  // Filtering logic
  const filteredRecords = mockNonAgentRecovery.filter((record) => {
  ...
  const confirmEndTask = () => {
    if (recordToEnd) {
      toast.success(`计划「${recordToEnd.planName}」的任务已结束`);
                    ...
                    <td className="sticky right-0 !bg-card z-10 shadow-[-6px_0_6px_-3px_rgba(0,0,0,0.05)] border-l transition-colors group-hover:!bg-accent text-sm">
                      <div className="flex items-center flex-wrap">
                        <button
                          ...
                          onClick={() =>
                            navigate(
                              `/dashboard/tasks/${taskId}/recovery-plan/${record.planIndex}/personnel`
                            )
                          }
                        >
                          任务人员回收详情
                        ...
                        <button
                          ...
                          onClick={() =>
                            navigate(
                              `/dashboard/tasks/${taskId}/recovery-plan/${record.planIndex}/subtasks`
                            )
                          }
                        >
                          查看子任务
