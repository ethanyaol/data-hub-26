...
const EditUser = () => {
    ...
    useEffect(() => {
        // Mock fetch existing user config
        if (id) {
            setTimeout(() => {
                setLoginName(`user_${id}`);
                setRealName(`预设模拟用户 ${id}`);
                setTenant("ai");
                setEmail(`user${id}@example.com`);
                setStatus("enabled");
                setAssignedRoles([{ id: "r1", name: "UAP管理员", description: "具备平台完整的菜单权限" }]);
                setLoading(false);
            }, 400);
        }
    }, [id]);
    ...
    if (loading) {
        return <div className="p-8 text-center text-muted-foreground animate-pulse">正在加载用户信息...</div>;
    }
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-foreground">编辑用户</span>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-4">
                ...
                {step === 2 && (
                    <Button onClick={handleFinish}>保存更改</Button>
                )}
            </div>
        </div>
    );
